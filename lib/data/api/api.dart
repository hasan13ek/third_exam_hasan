...
class MyApi {
  ...
  Future<List<CategoryItem>> getAllCachedCategoryItem() async {
    try {
      Response response = await https
          .get(Uri.parse("https://third-exam.free.mockoapp.net/categories"));
      if (response.statusCode == 200) {
        var jsonData = jsonDecode(response.body);
        var categories = (jsonData as List?)?.map((element) => CategoryItem.fromJson(element)).toList()??[];
        return categories;
      } else {
        throw Exception();
      }
  ...
  Future<List<ProductItem>> getCachedProductsId({required int id}) async {
    try {
      Response response = await https.get(
          Uri.parse("https://third-exam.free.mockoapp.net/categories/$id"));
      if (response.statusCode == 200) {
        var jsonData = jsonDecode(response.body);
        var products = (jsonData as List?)?.map((element) => ProductItem.fromJson(element)).toList() ?? [];
        return products;
      } else {
        throw Exception();
      }
